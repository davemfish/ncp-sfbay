### 17 Dec 2017 
### change output path to take a cl arg.
### for the SLR scenario 
### and change the path to write out the _pid.shp
...
if(length(args)!=2){
    print("userdays.r takes only 2 argument (aoi.shp, outputpath/)")
    stop
} else {
    aoipath <- args[1]
...
# writeOGR(AOIpolys, dsn=outpath, paste0(shpname, "_pid"), driver="ESRI Shapefile", overwrite=T)
write.csv(AOIpolys@data, file.path(outpath, paste0(shpname, "_pid")), row.names=F)
